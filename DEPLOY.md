...
1. UnivaPayの加盟店契約・ストアトークン（App Token + Secret）を取得。
2. Coolify の Environment Variables に設定：
   - `UNIVAPAY_ENABLED=true`
   - `UNIVAPAY_APP_JWT`（App Token JWT。checkout widgetでも使用＝公開可）
   - `UNIVAPAY_SECRET`（App Token Secret。バックエンド専用・秘匿）
   - `UNIVAPAY_STORE_ID`
   - `UNIVAPAY_WEBHOOK_TOKEN`（任意の長いランダム値）
3. 再デプロイ。
4. UnivaPay管理画面の Webhook 宛先に `https://<ドメイン>/webhook/univapay` を登録し、Authorization ヘッダに `UNIVAPAY_WEBHOOK_TOKEN` と同じ値を設定。
5. テストモードで申し込み→課金→`subscriptions.status=active`、`tenant.status=active` を確認。
