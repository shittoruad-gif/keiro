...
### A. UnivaPay 定期課金を有効化（2026-07-15 本番開通・実仕様で全面改訂）
...
**秘密情報の設定は環境変数ではなく、運営画面（/operator）の「決済設定（UnivaPay）」パネル推奨**
（暗号化してDB保存・起動時に反映・env より優先。SSH不要でスタッフが完結できる）。

1. **App Token（JWT）＋シークレット**: UnivaPay管理画面でアプリトークンを発行。
   - **URL(ドメイン)単位で発行**されるため、公開ドメイン（keiro.s-toru.com）を許可ドメインに指定
     （他ドメイン用トークンは流用不可）。
   - **発行時に一度だけ表示される「シークレット」を必ず控える**。サーバ側API（解約/照会）の認証は
     `Authorization: Bearer {secret}.{jwt}` で**シークレット必須**（JWT単体では401。公式docsで確認済み）。
   - ストアIDは**JWT内のクレームから自動導出**されるので手入力不要
     （手入力するとマーチャントIDと取り違える事故が実際に起きた）。
2. **決済リンク**: UnivaPay管理画面でプランごとに手動作成し、CoolifyのEnvironment Variablesに設定
   （URLは秘密情報ではないのでenvでよい）:
   - `UNIVAPAY_LINK_URL_LIGHT`（ライト・初回14日後課金） / `UNIVAPAY_LINK_URL_LIGHT_NOW`（ライト・当日課金）
   - `UNIVAPAY_LINK_URL_PRO`（プロ・初回14日後課金） / `UNIVAPAY_LINK_URL_PRO_NOW`（プロ・当日課金）
   - `UNIVAPAY_LINK_URL_PRO_30D`（プロ・初回30日後課金。パスコード適用のLINE構築客用）
   - 繰り延べリンクはトライアル中の先行カード登録用、当日課金リンクはトライアル失効後の申込用に自動出し分け。
3. **Webhook**: UnivaPay管理画面 →**左メニュー「ウェブフック」**（店舗設定の中ではない）→ウェブフック追加:
   - URL: `https://<ドメイン>/webhook/univapay`、利用店舗を指定、トリガー全チェック。
   - **Authorizationヘッダー欄に6文字以上の任意値**を設定し、**同じ値**を運営画面の「Webhook認証値」に保存。
   - **値は半角英数字のみ（記号・全角厳禁）**。IMEの全角「ｰ」混入でUnivaPay側の送信ごと失敗する実障害が起きた
     （運営画面は全角混入を保存時に拒否する）。
   - ※ 認証は**Authorizationヘッダーの固定値一致**（公式仕様）。HMAC署名は送られてこない。
   - 送信履歴は同画面の「最近のイベント」で確認できる。
4. 運営画面で4値（JWT/シークレット/ストアID自動/認証値）が揃うと課金連携は**自動で有効化**
   （`UNIVAPAY_ENABLED` は不要）。環境変数 `UNIVAPAY_JWT_TOKEN` / `UNIVAPAY_APP_SECRET` /
   `UNIVAPAY_STORE_ID` / `UNIVAPAY_WEBHOOK_SECRET` でも設定可能だがDB保存値が優先。
5. 動作確認: 決済リンクから申し込み（繰り延べリンクなら課金ゼロでテスト可能）→Webhook受信ログ
   `univapay webhook: subscription registered` →`subscriptions` 反映を確認。テスト分はAPIで解約すれば費用0円。
6. 料金は `PLAN_NAME` / `PLAN_AMOUNT`（円・税込）で設定。
