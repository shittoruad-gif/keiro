...
const { replyGreeting, replyText, replyMessages, pushMessages, getProfile: lineProfile } = require('./line');
    ...
    const items = list.map((c) => {
      ...
      return `
        <div class="coupon${expired ? ' expired' : ''}">
          ...
          <a class="btn" href="https://booking.moveact.net">今すぐ予約する →</a>
        </div>`;
    }).join('');
...
${items || '<div class="empty">現在利用できるクーポンはありません</div>'}
  ...
  app.post('/webhook/:token', limiter, express.raw({ type: '*/*' }), (req, res) => {
    ...
    const pendingIdentify = [];    // 自己申告フロー（クイックリプライ質問）
    for (const ev of events) {
      ...
      if (ev.type === 'postback' && lineUserId && ev.postback && ev.postback.data) {
        try {
          ...
          if (out && out.replyText && ev.replyToken) pendingAutoReplies.push({ replyToken: ev.replyToken, text: out.replyText });
      ...
      try {
        ...
        if (flow) pendingIdentify.push({ userId: lineUserId, flow });
    ...
    for (const r of pendingIdentify) {
      pushMessages(accessToken, r.userId, identify.buildQuickReplyMessages(r.flow))
  ...
  api.put('/bot-flows/:id/choices', (req, res) => {
  ...
  api.post('/bot-flows/seed-seitai', (req, res) => {
