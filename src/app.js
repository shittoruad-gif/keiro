...
${items || '<div class="empty">現在利用できるクーポンはありません</div>'}
  ...
  api.post('/reminders/:id/enroll', requirePro('reminders'), (req, res) => {
    ...
    const r = reminders.enroll(db, req.tenant.id, req.params.id, friend.line_user_id, b.base_date);
    if (r.error) return res.status(400).json(r);
    res.json(r);
  });
  ...
  const aiUsage = new Map(); // tenantId -> {day, count}（APIコスト保護: 1テナント10回/日）
  const AI_DAILY_LIMIT = 10;
  function aiQuotaOk(tenantId) {
    const day = new Date().toISOString().slice(0, 10);
    const u = aiUsage.get(tenantId);
    if (!u || u.day !== day) { aiUsage.set(tenantId, { day, count: 1 }); return true; }
  ...
  api.post('/ai-setup/analyze', async (req, res) => {
    ...
    if (!url) return res.status(400).json({ error: 'ホームページのURLを入力してください' });
    if (!aiQuotaOk(req.tenant.id)) return res.status(429).json({ error: '本日の解析回数の上限に達しました。明日また試すか、運営にご相談ください。' });
    const r = await aisetup.analyze(url);
    if (r.error) return res.status(400).json(r);
