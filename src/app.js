...
function createApp(db) {
  const app = express();
  // 信頼するプロキシは1段（Coolify/Traefikのリバースプロキシ）。true(全段信頼)にすると
  // クライアントが X-Forwarded-For を偽装して req.ip を乗っ取れ、レート制限を回避できる。
  ...
  // LINEトークンの能動疎通チェック: 毎時タイマー・8時台のみ・1日1回。
  // 失効（401/403）へ遷移したら院と運営へメール、回復したらフラグをクリア。
  // ネットワークエラーや5xxは一時障害の可能性があるため失効扱いにしない。
  let lastTokenCheckDay = null;
  async function checkLineTokens() {
    const d = new Date();
    const day = d.toDateString();
    if (d.getHours() !== 8 || lastTokenCheckDay === day) return;
    lastTokenCheckDay = day;
    const rows = db.prepare("SELECT * FROM tenants WHERE role='tenant' AND status='active' AND line_channel_access_token IS NOT NULL").all();
    for (const t of rows) {
      try {
        const token = tenantmod.resolveSettings(t).line.channelAccessToken;
        if (!token) continue;
        const r = await getBotInfo(token);
        if (r.ok) {
          if (t.line_token_error_at) {
            db.prepare('UPDATE tenants SET line_token_error_at = NULL WHERE id = ?').run(t.id);
            logger.info('line token recovered', { tenant_id: t.id });
          }
        } else if (r.http_status === 401 || r.http_status === 403) {
          if (!t.line_token_error_at) {
            db.prepare('UPDATE tenants SET line_token_error_at = ? WHERE id = ?').run(Date.now(), t.id);
            logger.warn('line token invalid', { tenant_id: t.id, status: r.http_status });
            mailer.sendMail({
              to: t.email,
              subject: '[Keiro] ⚠️ LINEとの接続が切れています（要ご確認）',
              text: `${t.name || ''} 様\n\nKeiroとLINE公式アカウントの接続が切れていることを検知しました。\nこのままでは自動返信・配信・計測が動きません。\n\nお手数ですが、Keiroの「かんたんスタート（LINE連携ウィザード）」の手順で、アクセストークンを再発行して貼り直してください。\n${config.baseUrl}/app\n\nご不明な場合は、アプリ内の「質問・サポート」からご連絡ください。すぐにお手伝いします。`,
            }).catch(() => {});
            if (config.operator.email) {
              mailer.sendMail({
                to: config.operator.email,
                subject: `[Keiro] ⚠️ LINEトークン失効: ${t.name || t.email}`,
                text: `院: ${t.name || ''}（${t.email}）\nHTTP ${r.http_status}\n院にも再設定案内メールを送信済み。フォローアップ推奨。`,
              }).catch(() => {});
            }
          }
        }
      } catch (e) { logger.error('token check error', { tenant_id: t.id, err: String((e && e.message) || e) }); }
    }
  }
  const tokenTimer = setInterval(() => checkLineTokens().catch((e) => logger.error('token check scheduler', { err: String(e && e.message || e) })), 60 * 60 * 1000);
  if (tokenTimer.unref) tokenTimer.unref();
  ...
  app.post('/connect/line', limiter, express.json({ type: () => true, limit: '64kb' }), (req, res) => {
    ...
    // 既に接続済みのアカウントのLINE認証情報が上書きされる場合は、乗っ取り検知のため運営へ通知。
    // （初回接続は正常な導線なので通知しない）
    const wasConnected = !!tenant.line_channel_access_token;
    ...
    if (overwriting && config.operator.email) {
      mailer.sendMail({
        to: config.operator.email,
        subject: `[Keiro] ⚠️ LINE接続情報が再設定されました: ${tenant.name || tenant.email}`,
        text: `テナント: ${tenant.name || ''}（${tenant.email}）のLINEアクセストークンが上書きされました。\nご本人による再接続であれば問題ありません。心当たりがない場合はご確認ください。`,
      }).catch(() => {});
    }
    res.json({ ok: true });
  });
...
${items || '<div class="empty">現在利用できるクーポンはありません</div>'}
  ...
  const LINE_EVENT_DEDUP_MS = 30 * 60 * 1000;
  const LINE_EVENT_DEDUP_MAX = 5000;
  function lineEventIsDup(id) {
    if (!id) return false;
    const now = Date.now();
    for (const [k, ts] of lineEventSeen) { if (now - ts > LINE_EVENT_DEDUP_MS) lineEventSeen.delete(k); else break; }
    if (lineEventSeen.has(id)) return true;
    lineEventSeen.set(id, now);
    if (lineEventSeen.size > LINE_EVENT_DEDUP_MAX) lineEventSeen.delete(lineEventSeen.keys().next().value);
    return false;
  }
  ...
  app.post('/webhook/univapay', express.raw({ type: '*/*' }), (req, res) => {
    ...
    try {
      ...
      // 契約ID（サブスクID）はサブスク由来のフィールドのみを採用する。
      // data.id はチャージイベントでは「チャージID」であり、これを契約IDとして保存すると
      // 毎月別レコードが増殖し解約APIも効かなくなるため、フォールバックには使わない。
      const univapaySubId = data.subscription_id
        || (data.subscription && data.subscription.id)
        || (data.transaction && data.transaction.subscription_id)
        || (/subscription/.test(eventType) ? (data.id || body.id) : null)
        || null;
      ...
      // 返金は「解約」とは別扱い（1回分の返金でテナントを停止しない）。運営へ通知のみ。
      const isRefund = /refund/.test(blob);
      const isCanceled = !isRefund && /(cancel|suspend)/.test(blob);
      const isFailed = !isRefund && !isCanceled && /(fail|error|declined|past_due|unpaid|chargeback)/.test(blob);
      const isPaidCharge = !isRefund && !isCanceled && !isFailed
        && /charge|payment/.test(eventType)
        && /(finish|success|paid|completed|captured|authorized|current)/.test(blob)
        && (chargeAmount ?? 0) > 0;
      const isSubscriptionStart = !isRefund && !isCanceled && !isFailed && !isPaidCharge;
      ...
      if (isCanceled) {
        ...
        // 返金: サブスク状態は変えず、運営へ通知のみ（必要なら手動対応）。
        logger.info('univapay webhook: refund', { tenant_id: tenant.id, type: eventType });
        mailer.sendMail({
          to: config.operator.email,
          subject: '[Keiro] 返金イベントを受信',
          text: `テナント: ${tenant.email}（${tenant.name || ''}）\ntype=${eventType}\n※契約状態は自動変更していません。必要に応じてご確認ください。\n生データ: ${JSON.stringify(body).slice(0, 1500)}`,
        }).catch(() => {});
      } else if (isSubscriptionStart) {
        // 既に有効な契約があるものを "trialing" に降格させない（支払い中の顧客の計測停止を防止）。
        if (univapaySubId && !(existing && existing.status === 'active')) {
          billing.upsertSubscription(db, { tenantId: tenant.id, univapaySubId, status: 'trialing' });
        }
        // カード初回登録（＝このテナントで初めての繰り延べサブスク作成）を受けて、無料期間を
        // 「登録日＋初回請求までの日数」に合わせて延長する。これにより無料期間終了〜初回請求の
        // 間に一時停止する隙間を無くす（＝実質「カード登録から◯日」で課金開始に揃う）。
        //   繰り延べ日数: プロ＋パスコード適用(LINE構築客)=30日 / それ以外(ライト・通常プロ)=14日
        if (!existing) {
          const deferralDays = (tenant.plan === 'pro' && tenant.code_redeemed) ? 30 : 14;
          const alignedEnd = Date.now() + deferralDays * 24 * 3600 * 1000;
          if ((tenant.trial_ends_at || 0) < alignedEnd) {
            db.prepare('UPDATE tenants SET trial_ends_at = ?, updated_at = ? WHERE id = ?').run(alignedEnd, Date.now(), tenant.id);
            logger.info('trial aligned to card registration', { tenant_id: tenant.id, deferral_days: deferralDays, trial_ends_at: alignedEnd });
          }
        }
      ...
      if (ev.type === 'message' && ev.message && ev.message.type === 'text' && ev.replyToken) {
        ...
        try {
          ...
          if (!isOwnerSelf && (!tenant.inbox_notice_at || Date.now() - tenant.inbox_notice_at > NOTICE_GAP_MS)) {
            ...
            const who = (fr && fr.display_name) ? fr.display_name + 'さん' : 'お客さま';
            const preview_ = String(ev.message.text).slice(0, 120);
  ...
  api.get('/me', (req, res) => {
    ...
    res.json({
      ...
      tutorial_seen_at: req.tenant.tutorial_seen_at || null,
    });
  });

  // 使い方ツアーを見終えた（スキップ含む）。以後の自動表示を止める
  api.post('/tutorial/done', (req, res) => {
    db.prepare('UPDATE tenants SET tutorial_seen_at = ? WHERE id = ?').run(Date.now(), req.tenant.id);
    res.json({ ok: true });
  });
  ...
  api.get('/billing/status', (req, res) => {
    ...
    // 永年無料（社内・特別付与）: 無料期間の残りが極端に長ければカード登録不要の永年扱い。
    // 閾値は約8年（3000日）。10年(3650日)コードの適用直後に境界割れで永年判定を外さないため。
    const permanent = st.inTrial && (st.trialEndsAt - Date.now()) > 3000 * 24 * 3600 * 1000;
    res.json({
  ...
  // ---- 通知先LINE（自分）の設定: 友だち行のIDで指定（一覧APIはline_user_idを伏せているため） ----
  api.post('/settings/owner-line', (req, res) => {
    const fid = String((req.body || {}).friend_id || '');
    const f = db.prepare('SELECT line_user_id, display_name FROM friends WHERE id = ? AND tenant_id = ?').get(fid, req.tenant.id);
    if (!f || !f.line_user_id) return res.status(404).json({ error: '友だちが見つかりません' });
    tenantmod.updateTenantSettings(db, req.tenant.id, { owner_line_user_id: f.line_user_id });
    logger.info('owner line set', { tenant_id: req.tenant.id });
    res.json({ ok: true, display_name: f.display_name || null });
  });

  // ---- 一斉配信のテスト送信（自分のLINEにだけ送って見え方を確認） ----
  api.post('/broadcasts/test', async (req, res) => {
    const text = String((req.body || {}).text || '').trim();
    const imageUrl = String((req.body || {}).image_url || '').trim() || null;
    if (!text) return res.status(400).json({ error: '本文を入力してください' });
    const owner = req.tenant.owner_line_user_id;
    if (!owner) return res.status(400).json({ error: 'owner_not_set' }); // フロントで設定ピッカーを開く
    const settings = tenantmod.resolveSettings(req.tenant);
    ...
    if (!r.ok) return res.status(400).json({ error: 'テスト送信に失敗しました（自分がこのLINEを友だち追加済みかご確認ください）', detail: r.response });
    logger.info('broadcast test sent', { tenant_id: req.tenant.id });
    res.json({ ok: true });
  });
  ...
  api.post('/redeem-code', codeLimiter, (req, res) => {
  ...
  api.post('/billing/cancel', async (req, res) => {
    ...
    // UnivaPay側の解約が成功したことを確認してからDBを解約にする。
    // 失敗（認証切れ/通信断/404）で「解約済み」にすると、実際は課金が続く事故になるため。
    if (univapay.enabled()) {
      const r = await univapay.cancelSubscription(sub.univapay_subscription_id);
      // 既に解約済み(404)は成功扱い。それ以外の失敗はエラーを返し、DBは変更しない。
      if (!r.ok && r.status !== 404) {
        logger.error('billing cancel failed at univapay', { tenant_id: req.tenant.id, status: r.status });
        return res.status(502).json({ error: '解約処理に失敗しました。お手数ですが時間をおいて再度お試しいただくか、サポートへご連絡ください。' });
      }
    }
  ...
  // ---- 空き枠おしらせ配信（予約システムの空き枠フィード連動・プロ） ----
  api.get('/vacancy', requirePro('vacancy'), (req, res) => {
    const s = vacancy.getSettings(db, req.tenant.id);
    const recent = db.prepare('SELECT ok, detail, created_at FROM vacancy_sends WHERE tenant_id = ? ORDER BY created_at DESC LIMIT 5').all(req.tenant.id);
    res.json({ settings: s, recent, default_template: vacancy.DEFAULT_TEMPLATE });
  });
  api.put('/vacancy', requirePro('vacancy'), (req, res) => {
    const r = vacancy.saveSettings(db, req.tenant.id, req.body || {});
    if (r.error) return res.status(400).json(r);
    logger.info('vacancy settings saved', { tenant_id: req.tenant.id, enabled: r.enabled });
    res.json(r);
  });
  // いま配信するとどう見えるか（送信はしない）
  api.get('/vacancy/preview', requirePro('vacancy'), async (req, res) => {
    const s = vacancy.getSettings(db, req.tenant.id);
    if (!s.feed_url) return res.status(400).json({ error: '先にフィードURLを設定してください' });
    try {
      const feed = await vacancy.fetchFeed(s.feed_url);
      const msg = vacancy.renderMessage(s, feed);
      res.json({ empty: msg.empty, text: msg.text, suppress_reason: vacancy.suppressReason(db, s, Date.now()) });
    } catch (e) {
      res.status(400).json({ error: String((e && e.message) || e) });
    }
  });
  // 自分のLINEに試し送り（お客さまには送らない）
  api.post('/vacancy/test', requirePro('vacancy'), async (req, res) => {
    const s = vacancy.getSettings(db, req.tenant.id);
    if (!s.feed_url) return res.status(400).json({ error: '先にフィードURLを設定してください' });
    const owner = req.tenant.owner_line_user_id;
    ...
    try {
      ...
      if (!r.ok) return res.status(400).json({ error: 'テスト送信に失敗しました', detail: r.response });
      res.json({ ok: true });
    } catch (e) {
      res.status(400).json({ error: String((e && e.message) || e) });
    }
  ...
  // ---- タグセグメント（タグのAND/OR組合せを保存→配信対象に使う） ----
  api.get('/segments', (req, res) => res.json(friends.listSegments(db, req.tenant.id)));
  api.post('/segments', (req, res) => {
    const b = req.body || {};
    const r = friends.createSegment(db, req.tenant.id, { name: b.name, mode: b.mode, tags: b.tags });
    if (r.error) return res.status(400).json(r);
    res.status(201).json(r);
  });
  api.delete('/segments/:id', (req, res) => res.json(friends.deleteSegment(db, req.tenant.id, req.params.id)));

  // ---- AI下書き（一斉配信/ステップ/自動応答の文面生成。反映は人間の保存操作が必須） ----
  api.post('/ai/draft', async (req, res) => {
    if (!aisetup.enabled()) return res.status(503).json({ error: 'AI機能は現在準備中です（運営にお問い合わせください）' });
    if (!aiQuotaOk(req.tenant.id, 'draft', 30)) return res.status(429).json({ error: '本日のAI下書き回数の上限に達しました。' });
    const b = req.body || {};
    const r = await aisetup.draftContent(db, req.tenant, { kind: b.kind, brief: b.brief });
    if (r.error) return res.status(400).json(r);
    logger.info('ai draft', { tenant_id: req.tenant.id, kind: b.kind });
    res.json(r);
  });
  ...
  api.get('/analytics/funnel', (req, res) => {
    const days = Math.min(Math.max(parseInt(req.query.days, 10) || 30, 1), 365);
    res.json(analytics.getFunnel(db, req.tenant.id, days));
  });
  ...
  api.get('/coupons/:id/uses', (req, res) => res.json(coupons.listUses(db, req.tenant.id, req.params.id)));
  api.post('/coupons/:id/mark-used', (req, res) => {
    const b = req.body || {};
    if (!b.friend_id && !b.use_id) return res.status(400).json({ error: '対象が指定されていません' });
    res.json(coupons.markUsed(db, req.tenant.id, req.params.id, { friendId: b.friend_id, useId: b.use_id }));
  });
  ...
  admin.post('/tenants/:id/:action', (req, res) => {
    ...
    // manual_hold: 手動停止は1（決済イベントで解除されない）、手動再開は0（以後は課金状態に追従）
    const manualHold = action === 'suspend' ? 1 : 0;
    const info = db.prepare("UPDATE tenants SET status = ?, manual_hold = ?, updated_at = ? WHERE id = ? AND role='tenant'").run(status, manualHold, Date.now(), id);
  ...
  admin.post('/billing-settings', (req, res) => {
    const b = req.body || {};
    // 全角文字の混入はHTTPヘッダーに載らず照合も送信も失敗する（実際にIMEの「ｰ」混入で障害発生）
    const FIELD_NAMES = { jwt: 'App Token（JWT）', app_secret: 'アプリトークンのシークレット', store_id: 'ストアID', webhook_secret: 'Webhook認証値' };
    for (const k of Object.keys(FIELD_NAMES)) {
      const v = String(b[k] || '').trim();
      if (v && !/^[\x21-\x7e]+$/.test(v)) {
        return res.status(400).json({ error: `${FIELD_NAMES[k]}に全角文字（かな・全角記号など）が含まれています。IMEを半角英数モードにして入力し直してください。` });
      }
    }
